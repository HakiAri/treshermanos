    ...
    if(isset($_REQUEST["user"])&&isset($_REQUEST["password"])){
        ...
        if ($result_of_login_check->num_rows == 1) {
              $result_row = $result_of_login_check->fetch_object();
                 if (password_verify($_REQUEST['password'], $result_row->password)) {
                    $usuario['id_user'] = $result_row->id_usuario;
                    $id = $result_row->id_usuario;
            ...
            $sqlToken = "UPDATE usuario SET token='{$token}' WHERE id_usuario='{$id}';";
            $resToken = $con->query($sqlToken);
        } else {
            //$this->errors[] = "Usuario y/o contraseña no coinciden.";
            $resultar["success"]=0;
        ...
        $resultar["success"]=0;
        $resultar["message"]='No Existen valores para la consulta';
        //$json['usuario'][]=$resultar;
        $json[]=$resultar;
        echo json_encode($json);
    }
